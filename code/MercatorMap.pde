...
public class MercatorMap {
  ...
  public PVector getLatLngFromPixel(PVector v){
    
    float lng = leftLongitudeRadians+((v.x*(rightLongitudeRadians-leftLongitudeRadians))/mapScreenWidth);
    float lat = topLatitudeRelative+((v.y*(bottomLatitudeRelative-topLatitudeRelative))/mapScreenHeight);
